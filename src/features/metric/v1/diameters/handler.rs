...
pub async fn diameters(Extension(pool): Extension<PgPool>, Query(params): Query<DiameterParams>) -> impl IntoResponse {
    ...
    match execute_query(&pool, query, move |q| q.bind(order), ResponseType::Multiple, None).await {
